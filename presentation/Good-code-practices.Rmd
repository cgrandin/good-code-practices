...
`+`(5, `*`(8, 10))
```
To prove this, look at the Abstract Syntax Trees (ASTs) that R creates when it parses an expression like this:
.pull-left[
...
---
class: center, middle, inverse
# Standardizing your syntax style
...
.left[
  ...
  - Any function can read or write to them which means the global state of the program can change in many ways at many times
...
.left[
  ...
  a <- 10
  b <- 20
  add2_topower <- function(pow = 2, a = a, b = b){
    add2(a, b) ^ pow
  }
  add2_topower()
  ```
