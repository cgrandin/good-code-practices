...
- Compilation is a process where the computer takes some code written in some language and generates machine
  code (a binary representation) in an executable file that can be run on a CPU
  
- Languages such as C, C++ are compiled with these steps:
  - Pre-processing: Pre-processing directives are used to modify the source or inject other code into
    the source prior to compilation. Also know as Pre-processor directives which look like this in C-type
    languages:
  ```c
    #include <iostream> 
  ```
  - Compiling: includes parsing the code and generating assembly language code which is targeted
    at a specific CPU
...
### Compilation (2)
  ...
  - Assembler: The assembler converts the generated assembly code into pure binary code, which is also known as     object code. When you compile C or C++ and get *.o files*, that's what is inside
  - Linking: Merges all object code from multiple modules into one. This includes any included libraries

  - An executable file is generated and this has to be run explicitly from the operating system
...
### Interpretation
 ...
 - Examples of interpreted languages are Javascript, Perl, Common Lisp, Java, and R
 
 - For R, the interpreter does the following steps:
 
   - Move to the next line/chunk of code and read it in:
   ```r
     x <- "Hello world"
   ```
   - Convert the line/chunk into a function call:
   ```r
   `<-`(x, "Hello world")
   ```
...
### Pros of Compilation over Interpretation (1)

- Execution time of interpreted code is **MUCH** slower than for compiled code
- Compilation allows full type checking ahead of time so you must define exactly what type a function's
  arguments must be and what type is returned
  - Interpretation allows a function argument to become any type which lends itself to fragile code
  - Lack of type-checking allows users to write functions which return a value even if an argument
    with the wrong type has been supplied to the function
...
### Pros of Interpretation over Compilation (2)

- Interpreted code is usually platform-independent where compiled code needs to be compiled for a specific CPU.
  Examples include Java's byte code and ELisp (Emacs Lisp) modules which are also a type of byte code

- Easier debugging: Direct source code debugging
...
### Paths and project location in the filesystem
...
- If using R, there's a package called **here** which detects the root directory for you, from
  which all relative paths will be based. The example above would be:
   ...
   ```
- To guarantee where the root directory is for the *here* package, place an empty *.here* file where you want that to be.
  All relative paths will be from that place
...
### For loops vs. lapply() and map()

- For loops are not slower and you can use them if you prefer
