...
```{c++ example-string-typing, echo = TRUE, eval = FALSE}
#include <iostream>
using namespace std;
...
mult(5, 3)
```
In this code, variables are typed and they could be anything. If we try to run the code with a different type something possibly unexpected occurs:
```{r mult-data-frame, echo = TRUE, eval = TRUE}
d <- data.frame(j = c(1, 2, 3), k = c(4, 5, 6))
mult(5, d)
```
Well, maybe that isn't so bad, but as the author of the function have you though of what will happen with **every** data type and dimension of those data types? What about this (nect page)
]
...
This is obvious here, but when errors like this happen inside someone else's package it is frustrating.

These errors can be caught at the beginning of the function by checking all the variables for correct type.
]
