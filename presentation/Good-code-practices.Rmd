...
- That will not prevent someone (or some function) from doing this however:
...
.pull-full[.left[
This is caused by default arguments being set with the form `j = j` in the function declaration and can be easily fixed by changing the argument declaration to `k = j`. Use a different variable name in the function than the default value.
]]
.pull-left[.left[
...
### How to fix promise errors with the ellipsis argument
.pull-left[.left[
...
### Tibbles 
Making a tibble from scratch is the same as making a `data.frame`. This code makes a tibble of numbers 10 raised to the power of random numbers between 0 and 1 and plots the distribution of the first digit of the number using `ggplot`:
.pull-left[.left[
